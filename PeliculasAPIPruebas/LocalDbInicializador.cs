...
namespace PeliculasAPIPruebas {
    ...
    public class LocalDbInicializador {
        ...
        public static ApplicationDBContext GetDbContextLocalDb(bool beginTransaction = true) {
            ...
            var servicioUsuario = new UsuarioService();

            var context = new ApplicationDBContext(options, servicioUsuario, eventosDbContextService: null);
            if (beginTransaction) {
        ...
        static void DeleteDB() {
            var fileNames = GetDbFiles(Master, $@"SELECT [physical_name] F
                                                  ROM [sys].[master_files]
                                                  WHERE [database_id] = DB_ID('{_dbName}')");

            if (fileNames.Any()) {
                ExecuteCommand(Master, $@"ALTER DATABASE [{_dbName}] 
